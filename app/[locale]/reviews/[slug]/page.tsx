...
export async function generateStaticParams() {
  ...
  for (const locale of locales) {
    const slugs = await getAllSlugsByType("reviews", locale);
    for (const slug of slugs) all.push({ locale, slug });
  }
  // When there are no reviews yet (or all are drafts), Next.js static export
...
export async function generateMetadata({
  ...
  let item;
  try {
    item = await getContentBySlug("reviews", slug, locale);
  } catch {
    return {};
  }
  const hasEn = await hasContentBySlug("reviews", slug, "en");
  const hasHu = await hasContentBySlug("reviews", slug, "hu");
...
export default async function ReviewDetailPage({
  ...
  const { locale: rawLocale, slug } = await params;
  const locale = normalizeLocale(rawLocale);
  return <ContentDetailPage locale={locale} type="reviews" slug={slug} />;
}
