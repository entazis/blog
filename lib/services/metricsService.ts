 ...
 * Adapted for Next.js with NEXT_PUBLIC_ environment variables
 */
...
class MetricsService {
  ...
  private loadConfigFromEnv(): void {
    ...
    const enabledEnv = process.env.NEXT_PUBLIC_METRICS_ENABLED;
    const enabled = enabledEnv === "false" ? false : true;

    // Parse and validate sampleRate with clamping to [0, 1]
    const sampleRateStr = process.env.NEXT_PUBLIC_METRICS_SAMPLE_RATE;
    const parsedSampleRate = sampleRateStr
      ? parseFloat(sampleRateStr)
    ...
    const batchSizeStr = process.env.NEXT_PUBLIC_METRICS_BATCH_SIZE;
    const parsedBatchSize = batchSizeStr
    ...
    const batchIntervalStr = process.env.NEXT_PUBLIC_METRICS_BATCH_INTERVAL;
    ...
    const envConfig: Partial<MetricsConfig> = {
      enabled,
      apiUrl:
        process.env.NEXT_PUBLIC_METRICS_API_URL || this.config.apiUrl,
      siteName: process.env.NEXT_PUBLIC_SITE_NAME || this.config.siteName,
      sampleRate,
      batchSize,
      batchInterval,
      debug: process.env.NEXT_PUBLIC_METRICS_DEBUG === "true"
    };
   ...
   * Track a page visit
   */
  public trackPageVisit(page: string, locale?: string): void {
    if (!this.isReady()) return;
  ...
  private handleSubmissionError(error: unknown, metrics: Metric[]): void {
    ...
    if (this.failedAttempts < this.maxRetries) {
      ...
      this.log(
        `Retrying... (attempt ${this.failedAttempts}/${this.maxRetries})`
      );
    } else {
